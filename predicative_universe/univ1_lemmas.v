...
Proof.
  revert a. pattern A ; eapply U1_rect ; clear A.
  - exact refl0.
  - exact biimpl_refl.
  - exact reflU0.
...
Proof.
  pattern A ; eapply U1_rect ; clear A ; try (now econstructor).
  intro A. exact (reflU0 A).
Defined.

Definition sym1_pre (A B : U1) {a : El1 A} {b : El1 B} : eq1 A B a b <--> eq1 B A b a.
Proof.
  revert B a b. pattern A ; eapply U1_rect ; clear A.
  - intro A. intro B ; pattern B ; eapply U1_rect ; clear B.
    + intros B a b. eapply (sym0_pre A B).
    + intros. split ; now intros [].
    + intros. split ; now intros [].
    + intros. split ; now intros [].
  - intro B ; pattern B ; eapply U1_rect ; clear B.
    + intros. split ; now intros [].
    + intros A B. split ; now eapply biimpl_sym.
    + intros. split ; now intros [].
    + intros. split ; now intros [].
  - intro B ; pattern B ; eapply U1_rect ; clear B.
    + intros. split ; now intros [].
    ...
    + intros. split ; now intros [].
  - intros A IHA P IHP Pe. intro B ; pattern B ; eapply U1_rect ; clear B.
    + intros a b. split ; now intros [].
    + intros a b. split ; now intros [].
    + intros a b. split ; now intros [].
    + intros B _ Q _ Qe f g. split.
      * intros e b a eba. eapply IHP. apply (andright (IHA _ _ _)) in eba. exact (e a b eba).
      * intros e a b eab. eapply IHP. apply (andleft (IHA _ _ _)) in eab. exact (e b a eab).
Defined.
...
Proof.
  revert B. pattern A ; eapply U1_rect ; clear A.
  - intro A. intro B ; pattern B ; eapply U1_rect ; clear B.
...
Record cast_lemmas_conclusion1@{i j k l} (A : U1@{i j k l}) (B : U1@{i j k l}) (e : eqU1 A B) : Type@{k} :=
  {
    transf1 : forall (C : U1@{i j k l}) {a b c} (eab : eq1 A B a b) (ebc : eq1 B C b c), eq1 A C a c ;
    transb1 : forall (C : U1@{i j k l}) {a b c} (eab : eq1 B A b a) (ebc : eq1 A C a c), eq1 B C b c ;
    castf1 : El1 A -> El1 B ;
    castb1 : El1 B -> El1 A ;
...
Definition cast_lemmas1@{i j k l} (A : U1@{i j k l}) (B : U1@{i j k l}) (e : eqU1 A B) : cast_lemmas_conclusion1@{i j k l} A B e.
Proof.
  eapply U1_rect2 ; clear A B e.
  - intros A B eAB. unshelve econstructor.
    + exact (cast0 A B eAB).
    + exact (castb (cast_lemmas A B eAB)).
    + intro C. pattern C ; eapply U1_rect ; clear C.
  ...
  - unshelve econstructor.
    ...
    + intro C. pattern C ; eapply U1_rect ; clear C.
      * easy.
      * intros A B C. eapply biimpl_trans.
      * easy.
      * easy.
    + intro C. pattern C ; eapply U1_rect ; clear C.
      * easy.
...
Definition trans1@{i j k l} (A : U1@{i j k l}) (B : U1@{i j k l}) (e : eqU1 A B) (C : U1@{i j k l}) {a b c} :
  eq1 A B a b -> eq1 B C b c -> eq1 A C a c.
Proof.
  exact (fun eab ebc => transf1 (cast_lemmas1 A B e) C eab ebc).
Defined.

Definition cast1@{i j k l} (A : U1@{i j k l}) (B : U1@{i j k l}) (e : eqU1 A B) :
  El1 A -> El1 B.
Proof.
  exact (castf1 (cast_lemmas1 A B e)).
Defined.

Definition cast1_eq@{i j k l} (A : U1@{i j k l}) (B : U1@{i j k l}) (e : eqU1 A B) :
  forall a, eq1 A B a (cast1 A B e a).
Proof.
  exact (castf1_eq (cast_lemmas1 A B e)).
Defined.

Definition transU1@{i j k l} {A B C : U1@{i j k l}} : eqU1 A B -> eqU1 B C -> eqU1 A C.
Proof.
  intro eAB. revert C. change ((fun A B eAB => forall C : U1, eqU1 B C -> eqU1 A C) A B eAB).
  eapply U1_rect2 ; clear eAB B A.
  - intros A B eAB C. pattern C ; eapply U1_rect ; clear C.
  ...
  - intros A B eAB IHA P Q ePQ IHP Pe Qe. intro C.
    pattern C ; eapply U1_rect ; clear C.
    + easy.
    + easy.
    + easy.
