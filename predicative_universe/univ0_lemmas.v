...
Proof.
  revert a. pattern A ; eapply U0_rect.
  - easy.
  - exact nateq_refl.
...
Proof.
  pattern A ; eapply U0_rect ; now econstructor.
Defined.

Definition sym0_pre (A B : U0) {a : El0 A} {b : El0 B} : eq0 A B a b <--> eq0 B A b a.
Proof.
  revert B a b. pattern A ; eapply U0_rect ; clear A.
  - intros P B ; pattern B ; eapply U0_rect ; clear B.
    2,3,4,5,6: intros ; split ; now intros [].
    intros Q p q. easy.
  - intro B ; pattern B ; eapply U0_rect ; clear B.
    1,3,4,5,6: intros ; split ; now intros [].
    intros a b. split ; exact nateq_sym.
  - intros A IHA P IHP Pe. intro B ; pattern B ; eapply U0_rect ; clear B.
  ...
  - intros A IHA P IHP Pe. intro B ; pattern B ; eapply U0_rect ; clear B.
    ...
    + intros e b a eba. eapply IHP. eapply (andright (IHA _ _ _)) in eba. exact (e a b eba).
    + intros e a b eab. eapply IHP. eapply (andleft (IHA _ _ _)) in eab. exact (e b a eab).
  - intros A IHA P IHP Pe. intro B ; pattern B ; eapply U0_rect ; clear B.
  ...
  - intros A IHA R Re. intro B ; pattern B ; eapply U0_rect ; clear B.
...
Proof.
  revert B. pattern A ; eapply U0_rect ; clear A.
  - intro P. intro B ; pattern B ; eapply U0_rect ; clear B.
    2,3,4,5,6: split ; now intros [].
    intro Q. split ; eapply biimpl_sym.
  - intro B ; pattern B ; eapply U0_rect ; clear B.
  ...
  - intros A IHA R Re. intro B ; pattern B ; eapply U0_rect ; clear B.
    ...
    + intros [ eAB eRS ]. apply IHA in eAB. econstructor.
      exact eAB. intros a0 b0 e0 a1 b1 e1. eapply biimpl_sym. eapply eRS.
      now eapply sym0. now eapply sym0.
    + intros [ eBA eSR ]. apply IHA in eBA. econstructor.
      exact eBA. intros a0 b0 e0 a1 b1 e1. eapply biimpl_sym. eapply eSR.
      now eapply sym0. now eapply sym0.
Defined.
...
Record cast_lemmas_conclusion@{i j k} (A : U0@{i j k}) (B : U0@{i j k}) (e : eqU0 A B) : Type@{j} :=
  {
    transf : forall (C : U0@{i j k}) {a b c} (eab : eq0 A B a b) (ebc : eq0 B C b c), eq0 A C a c ;
    transb : forall (C : U0@{i j k}) {a b c} (eab : eq0 B A b a) (ebc : eq0 A C a c), eq0 B C b c ;
    castf : El0 A -> El0 B ;
    castb : El0 B -> El0 A ;
...
Definition closure_eqrel (A B : U0) (eAB : eqU0 A B) (IH : cast_lemmas_conclusion A B eAB)
  (R : El0 A -> El0 A -> Type) (S : El0 B -> El0 B -> Type)
  ...
  : forall a0 b0 (e0 : eq0 A B a0 b0) a1 b1 (e1 : eq0 A B a1 b1), closure (eq0 A A) R a0 a1 <--> closure (eq0 B B) S b0 b1.
Proof.
  intros a0 b0 e0 a1 b1 e1. split.
  - intro Ha. revert b0 e0 b1 e1. induction Ha as [ a b H | a b H | H | H ].
    + intros b0 e0 b1 e1. eapply clo_emb. eapply (eRS a b0 e0 b b1 e1). exact H.
    + intros b0 e0 b1 e1. pose proof (transb IH B (transb IH A (sym0 _ _ e0) H) e1) as be.
  ...
  - intro Hb. revert a0 e0 a1 e1. induction Hb as [ a b H | a b H | H | H ].
    + intros a0 e0 a1 e1. eapply clo_emb. eapply (eRS a0 a e0 a1 b e1). exact H.
    + intros a0 e0 a1 e1. pose proof (transf IH A (transf IH B e0 H) (sym0 _ _ e1)) as ae.
...
Definition cast_lemmas@{i j k} (A : U0@{i j k}) (B : U0@{i j k}) (e : eqU0 A B) : cast_lemmas_conclusion@{i j k} A B e.
Proof.
  ...
  - unshelve econstructor.
    + eapply e.
    + eapply e.
    + intro C. pattern C ; eapply U0_rect ; clear C ; easy.
    + intro C. pattern C ; eapply U0_rect ; clear C ; easy.
    + easy.
    + easy.
  (* nat *)
...
Definition trans0@{i j k} (A : U0@{i j k}) (B : U0@{i j k}) (e : eqU0 A B) (C : U0@{i j k}) {a b c} :
  eq0 A B a b -> eq0 B C b c -> eq0 A C a c.
Proof.
  exact (fun eab ebc => transf (cast_lemmas A B e) C eab ebc).
Defined.

Definition cast0@{i j k} (A : U0@{i j k}) (B : U0@{i j k}) (e : eqU0 A B) :
  El0 A -> El0 B.
Proof.
  exact (castf (cast_lemmas A B e)).
Defined.

Definition cast0_eq@{i j k} (A : U0@{i j k}) (B : U0@{i j k}) (e : eqU0 A B) :
  forall a, eq0 A B a (cast0 A B e a).
Proof.
  exact (castf_eq (cast_lemmas A B e)).
Defined.

Definition transU0@{i j k} {A B C : U0@{i j k}} : eqU0 A B -> eqU0 B C -> eqU0 A C.
Proof.
  ...
  - intros P Q ePQ. intro C.
    pattern C ; eapply U0_rect ; clear C ; try easy.
    intros R eQR. exact (biimpl_trans ePQ eQR).
  - easy.
  ...
  - intros A B eAB IHA R S eRS Re Se. intro C.
    ...
    + intros a0 c0 eac0 a1 c1 eac1. set (b0 := cast0 A B eAB a0). set (b1 := cast0 A B eAB a1).
      ...
      exact (biimpl_trans (eRS a0 b0 eab0 a1 b1 eab1) (eST b0 c0 ebc0 b1 c1 ebc1)).
Defined.
